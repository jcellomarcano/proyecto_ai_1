...
void establishHeuristic(){
	FILE *f1 = fopen("TopSpin-14-4_abst1pdb.pdb","r");
	FILE *f2 = fopen("TopSpin-14-4_abst2pdb.pdb","r");


	pdb1 = read_state_map(f1);
	pdb2 = read_state_map(f2);

	abs1 = read_abstraction_from_file("TopSpin-14-4_abst1pdb.abst");
    abs2 = read_abstraction_from_file("TopSpin-14-4_abst2pdb.abst");

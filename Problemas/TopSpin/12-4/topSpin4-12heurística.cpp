...
void establishHeuristic(){
	FILE *f1 = fopen("TopSpin-12-4_abst1.pdb","r");
	FILE *f2 = fopen("TopSpin-12-4_abst2.pdb","r");
	
	pdb1 = read_state_map(f1);
    pdb2 = read_state_map(f2);

    abs1 = read_abstraction_from_file("TopSpin-12-4_abst1.abst");
	abs2 = read_abstraction_from_file("TopSpin-12-4_abst2.abst");

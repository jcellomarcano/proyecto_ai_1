...
void establishHeuristic(){
	FILE *f1 = fopen("THanoi-4-18_abst1PDB.pdb","r");
	FILE *f2 = fopen("THanoi-4-18_abst2PDB.pdb","r");


	pdb1 = read_state_map(f1);
	pdb2 = read_state_map(f2);

	abs1 = read_abstraction_from_file("THanoi-4-18_abst1PDB.abst");
    abs2 = read_abstraction_from_file("THanoi-4-18_abst2PDB.abst");
